...
; Source - https://stackoverflow.com/a/70371884
; Posted by user3010850, modified by community. See post 'Timeline' for change history
...
(defun hamza/unki (task-name) ;difficulty
  ;; "Creates custom spaced free recall revisions in ORG-MODE based on modern learning methods. the DIFFICULTY argument describes the difficulty of the content to free recall. It increases the number of repetitions in any given time frame."
  ;; (interactive "sDifficulty (Easy: -1, Normal: 0, Hard: 1): ")
  (interactive "sWhat is your task (also include a range of 'units' covered)? ")
  (let* ((repetitions `(("." . ,(make-decoded-time))
                        ("d" . ,(make-decoded-time :day 1))
                        ("w" . ,(make-decoded-time :day 7))
                        ("m" . ,(make-decoded-time :month 1))
                        ("3m" . ,(make-decoded-time :month 3))
                        ("y" . ,(make-decoded-time :year 1))))
         (repetition-times (mapcar #'cdr repetitions)))
    (org-insert-heading nil t 1)
    ;; (org-insert-timestamp (current-time) nil nil nil (concat " " task-name))
    (insert task-name)
    (cl-loop for idx from 1
             for (delta-label . delta-time) in repetitions
             do (progn
                  (org-insert-heading nil t 2)
                  (insert (format " [%s] %s" delta-label task-name))
                  (org-schedule nil (encode-time
                                     (seq-reduce
                                      (lambda (total elem)
                                        (decoded-time-add total elem))
                                      (take idx repetition-times)
                                      (decode-time))))))))
